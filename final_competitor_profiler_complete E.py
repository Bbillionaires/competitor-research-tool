...
def check_directory_citations(business_name: str, domain: str) -> dict:
    ...
    # Simple heuristic: if they have good reviews, likely listed on directories
    # Full implementation would use Google CSE to check each directory
    citation_count = 0
    
    # Simplified estimation based on domain maturity
    if hash(domain) % 3 == 0:
...
def main() -> None:
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {
            executor.submit(process_competitor, url, query): url 
        ...
        for future in as_completed(futures):
            try:
                ...
                if row and row["domain"] not in seen_domains:
                    ...
                    rows.append(row)
                    print(f"  ✓ {row['domain']} (Score: {row.get('competitor_score', 0)})")
            except Exception as e:
