...
def run_base_profiler(query, max_results):
    ...
    try:
        # Try to pass as command-line arguments first
        process = subprocess.Popen(
            [sys.executable, profiler, '--query', query, '--max-results', str(max_results)],
        ...
        # If it failed, try the stdin method
        if process.returncode != 0:
            print("  ℹ️  Trying stdin method...")
            process = subprocess.Popen(
